...
def get_lip_distance(landmarks, upper_lip_idx, lower_lip_idx, frame_w, frame_h):
    ...
    upper_lip_points = np.array(
        [(landmarks[i].x * frame_w, landmarks[i].y * frame_h) for i in upper_lip_idx]
    )
    lower_lip_points = np.array(
        [(landmarks[i].x * frame_w, landmarks[i].y * frame_h) for i in lower_lip_idx]
    )
    return np.linalg.norm(
        np.mean(upper_lip_points, axis=0) - np.mean(lower_lip_points, axis=0)
    )
...
def generate_frames():
    ...
    while True:
        ...
        cv2.putText(
            frame, gaze_text, (10, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (147, 58, 31), 2
        )
        ...
        if result.multi_face_landmarks:
            for landmarks in result.multi_face_landmarks:
                distance = get_lip_distance(
                    landmarks.landmark, UPPER_LIP, LOWER_LIP, w, h
                )
        ...
        cv2.putText(
            frame, status, (10, 210), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2
        )
...
def calibration_stream():
    ...
    while True:
        ...
        cv2.putText(
            frame, txt1, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (133, 199, 125), 1
        )
        cv2.putText(
            frame, txt2, (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (133, 199, 125), 1
        )
        cv2.putText(
            frame, txt3, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (133, 199, 125), 1
        )
...
def calibration():
    ...
    return Response(
        calibration_stream(), mimetype="multipart/x-mixed-replace; boundary=frame"
    )
